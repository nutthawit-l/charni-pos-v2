...
export function StoreActionBar({ hasChanges, isSaving, onSave, onNewProduct }: StoreActionBarProps) {
    return (
        <div className="flex shrink-0 gap-4 border-t border-outline-variant bg-surface px-2 py-4">
            <button
                type="button"
                onClick={onNewProduct}
                className="flex-1 rounded-full bg-surface-container px-6 py-3 text-sm font-medium text-on-surface-variant"
            >
                New Product
            </button>
            <button
